...
function Mat(m:number, n:number, v=<number[]>[]) {
    ...
    if (v.length>m*n) throw "The number of provided values exceeds the matrix capacity!";
...
Mat.submat = (A:Matrix, cell:Vector2) => {
    ...
    for (let y=0; y<m; y++) { // rows
        for (let x=0; x<n; x++) { // cols
            if (y!==cell[0]&&x!==cell[1]) r.push(A.mat[y*n+x]);
        }
    }
    return Mat(m-1, n-1, r);
}

Mat.sec2sel = (A:Matrix, ...sec:MatrixSection[]): number[] => {
    let r=<number[]>[];
...
Mat.sel2mat = (A:Matrix, newDim:Vector2, sel:number[]) => {
    let r=[];
...
Mat.squareSubmats = (A:Matrix, order:number) => {
    let r=<number[][]>[];
...
Mat.det = (A:Matrix) => {
    const m=A.dim[0];
    if (!Mat.isSquare(A)) throw "Incompatible matrix!";
    else if (m===0) return 1;
    else if (m===1) return A.mat[0]; // A
    else if (m===2) return A.mat[0]*A.mat[3]-A.mat[1]*A.mat[2]; // ad - bc
    else {
        let t = 0;
